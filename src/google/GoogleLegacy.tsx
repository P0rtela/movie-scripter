...
const GoogleComponent = () => {
    ...
    useEffect(() => {
        ...
        const checkAndCreateFolder = async (folderName: any) => {
        ...
        const returnFileNamesAndIndexes = async (folderName: any) => {
            const folderQuery = `name='${folderName}' and mimeType='application/vnd.google-apps.folder'`;
            const folderResponse = await gapi.client.drive.files.list({ q: folderQuery });
            const folders = folderResponse.result.files;
            ...
            if (folders && folders.length > 0) {
                ...
                if (files && files.length > 0) {
                    ...
                    files.forEach((file: { name: any; }, index: any) => {
        ...
        const readFileByIndexOrDefault = async (folderName: any, index: number) => {
            const folderQuery = `name='${folderName}' and mimeType='application/vnd.google-apps.folder'`;
            const folderResponse = await gapi.client.drive.files.list({ q: folderQuery });
            const folders = folderResponse.result.files;
